...
- 当前用户资料 HTTP 接口与 `t_user_info` 仍未实现
- 当前基线：`mvn clean test` 通过（198 tests，0 failures，0 errors，2 skipped；Docker 不可用，`MySqlFlywayMigrationTest` 与 `MySqlLoginFailureConcurrencyTest` 跳过）

**当前处置方案**：按 identity → system → content → comment → stats / common-infra 顺序重建。

## 3. 下一步推进顺序（详见 `roadmap.md`）

1. 评审 identity 是否继续实现 `t_user_info`，或按既定顺序转入 system 模块
2. 按模块重建：system → content → comment → stats / common-infra；`t_user_info` 可作为 identity 独立小批次
3. 前台 / 后台前端骨架
