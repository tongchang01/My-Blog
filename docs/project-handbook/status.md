...
- M3 已允许进入；identity 后台登录最小纵向切片已完成账号读取、凭据校验、失败累计、持久化锁定和成功登录状态更新
- 下一步设计并实现 Caffeine 登录限流，再进入双 token 登录事务编排与 Controller
- 当前基线：`mvn clean test` 通过（132 tests，0 failures，0 errors，2 skipped；Docker 不可用，两个 Testcontainers MySQL 测试跳过）
