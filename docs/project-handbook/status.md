...
- 当前基线：`mvn clean test` 通过（262 tests，0 failures，0 errors，2 skipped；Docker 不可用，`MySqlFlywayMigrationTest` 与 `MySqlLoginFailureConcurrencyTest` 跳过）

**当前处置方案**：按 identity → system → content → comment → stats / common-infra 顺序重建。

## 3. 下一步推进顺序（详见 `roadmap.md`）

1. 单独设计并实现 identity 修改密码，明确修改后 access / refresh token 的失效语义
2. identity 收尾后按模块重建：system → content → comment → stats / common-infra
3. 前台 / 后台前端骨架
