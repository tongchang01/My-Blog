...
- identity 后端模块已完成；当前基线：`mvn clean test` 通过（288 tests，0 failures，0 errors，4 skipped；跳过项均为 Docker 不可用时的 Testcontainers MySQL 条件测试）

**当前处置方案**：identity 已收尾，按 system → content → comment → stats / common-infra 顺序继续重建。

## 3. 下一步推进顺序（详见 `roadmap.md`）

1. 设计并实现 system 的 `t_site_config` 单行配置查询 / 编辑纵向切片
2. 继续完成 system 的 `t_attachment` 与 `t_friend_link`，再进入 content → comment → stats / common-infra
3. 前台 / 后台前端骨架
