...
- M3 已进入业务模块重建；identity 后台登录最小纵向切片已经提供真实 `POST /api/auth/login`
...
- refresh token 轮换、撤销和 token version 失效底层能力已存在，但 refresh / logout / 当前用户资料 HTTP 接口尚未开放
- 当前基线：`mvn clean test` 通过（169 tests，0 failures，0 errors，2 skipped；Docker 不可用，`MySqlFlywayMigrationTest` 与 `MySqlLoginFailureConcurrencyTest` 跳过）

**当前处置方案**：按 identity → system → content → comment → stats / common-infra 顺序重建。

## 3. 下一步推进顺序（详见 `roadmap.md`）

1. 评审 identity 下一批范围：refresh / logout Controller，或 `t_user_info` 用户资料能力
