...
- 表归属哪个模块？（identity / content / comment / system / stats / common-infra，见 `arch/module-map.md`）
- 表是否落在已锁定的 ~13 张表清单内？若是新增项，先回写 `product/decisions-draft.md` + `arch/schema-design.md` 再写 SOP
...
- 🔴 V2 不沿用 V1 表结构（ADR-0013），新表按 `arch/schema-design.md` 规范设计
- 🔴 不要在 Flyway 里写业务数据（业务数据用 seed/管理后台维护）
- 🔴 不要修改已 apply 过的 Flyway 脚本，加 `V{n+1}__fix_xxx.sql` 修补
- 🔴 新增模块表后同步更新 `arch/module-map.md` + `ArchitectureRulesTest`
