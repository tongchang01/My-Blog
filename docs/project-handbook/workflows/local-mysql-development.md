...
建议创建仅管理本地 V2 数据库的账号。以下密码必须替换为你自己的本机密码，不得提交真实值：
...
$env:MYBLOG_DATASOURCE_USERNAME = "myblog_dev"
$env:MYBLOG_DATASOURCE_PASSWORD = "<LOCAL_ONLY_DATABASE_PASSWORD>"
$env:MYBLOG_JWT_SECRET = "<LOCAL_ONLY_JWT_SECRET_AT_LEAST_32_BYTES>"
$env:MYBLOG_STATS_HASH_SECRET = "<LOCAL_ONLY_STATS_HASH_SECRET>"
```

不要把这些值写入 `.env`、YAML、脚本、终端截图或 Git 提交。初始化脚本使用 `MYSQL_PWD` 临时传递密码，并在退出前恢复原值。

## 三、初始化或重建数据库

首次建立空库，或明确丢弃现有本地 V2 数据时：
...
`-Reset` 会且只会删除并重建 `myblog_v2_dev`。脚本拒绝其他数据库名，随后隐藏启动 Spring Boot，让 Flyway 应用到版本 2，再导入 `seed.sql` 并执行验收查询。
