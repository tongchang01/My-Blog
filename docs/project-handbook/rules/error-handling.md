...
**错误码空间（与 `api-response.md` §7 一致）**：10 / 20 / 30 / 40 / 50
段是模块码预留空间，不代表每个业务错误都必须新增模块专用码。
...
- **`9X` 段预留基础设施**，业务模块禁止占用
- 首版优先复用语义稳定的通用错误码及认证授权码。
- 只有前端需要针对某一业务原因执行稳定分支时，才新增模块码；新增后必须同步
  对应 API contract 和自动化测试。
- 新增模块码时，模块内 `SSS` 从 `001` 起递增，不复用编号。
- Java 枚举名按 `{MODULE}_{SUBJECT}_{REASON}` 命名（如 `CONTENT_ARTICLE_NOT_FOUND`），便于代码阅读；返回给前端的是 `code` 字符串。

新增错误码 checklist：
1. 确认前端确实需要对此原因做稳定分支，且现有稳定语义码无法表达
2. 选定 MM 段（按所属模块）
3. 在对应模块下取下一个未使用的 SSS
4. 配 HTTP 状态码（按 §4）并写中文默认消息
5. 同步 API contract 和自动化测试
