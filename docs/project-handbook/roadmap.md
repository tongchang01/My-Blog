...
- [x] 更新 `application.yml`（MySQL `serverTimezone=Asia/Tokyo` / Jackson JST / Knife4j 4.x；API 文档仅 `local` / `test` 开启）
- [ ] ArchUnit 规则按新 6 模块（identity / content / comment / system / stats / common-infra）调整
- [ ] M2 收尾时评估 Maven Enforcer，锁定 Java / Maven 版本并检查依赖收敛

### M3：模块重建（按新 schema）

- [ ] identity：t_user_auth / t_user_info / t_refresh_token + 双 token 流程；实现登录限流时引入 Caffeine
...
- [ ] 首个 DTO / Entity 转换落地时引入 MapStruct；首个 Mapper 集成测试落地时引入 Testcontainers MySQL
