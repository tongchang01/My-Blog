...
- [ ] identity：认证会话与当前用户资料已完成。已落地 `t_user_auth` 登录、Caffeine 限流、双 token、refresh 行锁轮换、全端退出、`t_user_info`、GET `/api/auth/me` 和 ADMIN 资料 PATCH；剩余修改密码
...
- [ ] 首个 DTO / Entity 转换落地时引入 MapStruct
