...
- [x] content：分类标签、文章核心、公开查询、软删除回收站、恢复引用校验与定时发布已完成；评论前置能力待后续模块接入
- [x] comment：t_comment（含 content_md / content_html 双存）、公开评论、留言板、后台审核、文章评论计数、Resend 邮件失败日志、CommonMark + OWASP HTML Sanitizer 已完成
- [x] system：`t_site_config`、`t_attachment`、`t_friend_link` 已完成
- [x] stats：公开打点、每日 HMAC 访客标识、日 PV/UV 聚合、启动补算、5 分钟校准、90 天清理和 ADMIN/DEMO 总览已完成
- [x] common-infra：邮件端口、Resend 适配、失败日志持久化已随 comment 切片落地；后续如新增跨模块能力另开小批次
- [x] 首个 DTO / Entity 转换已在分类标签切片引入 MapStruct 1.6.3
- [x] 已引入 Testcontainers MySQL，用于 Flyway 真实方言验证；后续首个业务 Mapper 集成测试复用
- [x] 首版 PASSWORD 锁定态与 DEMO 敏感字段裁剪边界冻结；完整解锁仍位于 L2

### M4：前端骨架

> M3 后端六模块已全部完成，当前下一主线为 M4 前台 / 后台骨架。

- [x] frontend-user 首批：Vue 3 + Pinia + TypeScript + vue-i18n 三语路由、站点配置、公开文章列表与详情
- [ ] frontend-user 后续：分类/标签/归档/友链/关于/搜索、评论/留言/统计和 PASSWORD 解锁
- [x] frontend-admin 基础闭环：Pure Admin Thin 固定快照、三语、静态权限、认证会话、空仪表盘
- [x] frontend-admin 文章首批：独立筛选卡片、只读列表、三语展示、ADMIN/DEMO、完整请求状态
- [ ] frontend-admin 文章后续：详情/编辑、Vditor、ADMIN 写操作与 DEMO 禁用交互
- [ ] frontend-admin 其他业务页：分类/标签、评论、友链、配置和真实统计仪表盘
...
- [x] 评论 @ 回复邮件通知（Resend 默认关闭，失败写 `t_mail_log`）
- [ ] 文章访问 TOP 10 / 活动热图
