...
- 上游假通知、孤立 Welcome 页面和页头命令面板已删除；命令面板专属的拼音搜索、收藏拖拽依赖、配置、三语文案和资源同步移除。403/404/500 路由保留但不进入业务菜单。
- 无业务使用的 iframe 外壳、对应路由类型和多页处理已删除；业务页通过现有 RouterView、过渡、缓存、页脚及轻量的响应式路由组件交接层渲染。打印功能的临时 iframe 与通用样式规则保留。
- 三种布局、多标签页、动态菜单和主题能力仍复用上游实现，后续按独立任务逐项确认；命令面板和 iframe 外壳清理均未改变业务页面、路由或列表筛选。
