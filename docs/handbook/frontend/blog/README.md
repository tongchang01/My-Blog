...
- 播放器挂载在路由视图外的常驻顶部导航中，音乐图标位于语言切换左侧；未配置歌单时不显示入口，读者主动展开后才加载 Spotify。折叠保留 iframe，停止并关闭会销毁它，重新加载会创建新实例。
...
- 当前布局预览在桌面端由音乐图标下方靠右展开标准播放器；移除原有右下角入口、大段说明和宽外框，保留外部打开、重载与停止的小型操作栏。点击面板外部或在博客侧控件按 Escape 仅收起；主动停止和 Escape 收起会将焦点返回音乐入口。Spotify iframe 内的键盘事件由 Spotify 自身处理。
- 小屏幕使用底部面板布局，尚未完成真实设备验收。布局与样式仍可继续调整，不视为最终发布方案，见 [ISSUE-010](../../start-here/open-issues.md)。本地验证与发布状态见[当前状态](../../start-here/current-status.md)。
