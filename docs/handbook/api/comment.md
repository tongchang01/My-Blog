...
分页 `total` 只统计公开根评论；当前页根评论的 `replies` 完整返回，回复暂不单独分页。根评论被隐藏或删除后，整棵回复树不再公开，但回复自身的审核和删除状态保持不变；根评论重新通过或恢复后，仍为 `PASS + 未删除` 的回复自动恢复展示。
...
文章 `commentCount` 统计公开可达的评论数。审核、隐藏、删除、恢复和回复创建会先锁定所属根评论，再锁定目标回复；根评论失效时按整棵公开回复树调整计数，但不批量修改回复状态。该锁顺序同时避免根评论状态变化与回复审核、创建并发时产生计数偏差。
