...
| V2 前台 blog | 主流程基本完成，友链简版待补 | 站点配置、首页槽位、文章列表、文章详情、分类、标签、归档、关于、搜索、访问统计和文章评论已接入；友链仍走旧数据源 |
...
- 友链简版：接入 `GET /api/public/friend-links`，展示公开友链列表。
...
1. 补齐友链简版，消除前台最后一个明显旧 JSON 页面。
2. 收口部署硬项：生产环境变量、反向代理、CORS、客户端 IP、S3、备份恢复和上线冒烟。
3. 手动部署跑通后再设计 CD。
4. 第一版发布后再排 PASSWORD 完整解锁、留言板评论、SEO 增强和旧第三方评论代码清理。
