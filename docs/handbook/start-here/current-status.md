...
- DEMO 敏感字段裁剪需逐页确认。
- `topArticles[].articleId` 后端仍返回 JSON number，前端模型按 string 处理，需在 O-012 中裁决。

未完成项以 `open-issues.md` 的 O-002、O-012 为准。
...
2. 接入前台评论、留言和访问统计。
3. 裁决 DEMO 字段裁剪、公开 ID 类型和统计 TOP 文章 ID 契约。
4. 上线准备：SEO、RSS、Sitemap、部署、备份和 CI/CD。
5. 继续收口文档入口状态、产品规格和运维细节。
