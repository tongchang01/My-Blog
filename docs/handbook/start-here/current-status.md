...
- 评论审核写路径已按讨论串根节点串行化，但尚待 CI 的真实 MySQL 并发回归确认；首页槽位 1/2 上限仍缺少原子持久化约束。
...
- 后端 `mvn clean test` 完整验证通过：689 项测试，0 failures / 0 errors / 7 skipped；真实 MySQL 评论并发测试已增加根评论隐藏与回复审核竞争，本机因没有 Docker 跳过，待 CI 验证。
