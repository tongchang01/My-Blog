...
该批修复已通过完整 CI、发布 GHCR 镜像并部署生产，三条公开 HTTPS 健康端点均返回 `ok`。基础安全响应头已完成仓库侧实现；令牌 Cookie 化、CSP、自动回滚和管理端模板瘦身等审查建议尚未实现，不因本次修复而视为完成。
...
- 静态博客、`www` 和管理端由 Caddy 补齐 `nosniff`、Referrer Policy、同源 frame 限制和未使用浏览器能力限制；`/api` 已有的 Spring Security 更严格响应头不会被覆盖。
- shell 合约与 Caddy 2.11.4 配置语法验证通过；CI 还会使用仓库固定的同版本 Caddy 镜像复验。
- 以上仅是仓库侧证据；生产生效只以对应 `main` SHA 的 CD 对三个站点及公开 API 的响应头检查为准，本次本地实现不声明线上已经更新。CSP Report-Only 与强制策略仍未实现。
