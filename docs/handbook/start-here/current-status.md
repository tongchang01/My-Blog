...
- 评论审核写路径已增加行锁，但尚待 CI 的真实 MySQL 并发回归确认；根评论失效规则仍可能使文章评论计数偏离公开树，首页槽位 1/2 上限也缺少原子持久化约束。
...
- 后端评论应用层与 H2 持久层定向回归通过：10 项测试，0 failures / 0 errors；新增真实 MySQL 评论并发测试，本机因没有 Docker 跳过，待 CI 验证。
- 后端全量 `mvn test` 验证通过：685 项测试，0 failures / 0 errors / 6 skipped；其中真实 MySQL 测试需在有 Docker 的 CI 环境执行。
