...
| 范围 | 当前状态 |
| --- | --- |
| 后端 | 五个业务模块与 common 已实现；Flyway V1–V5、15 张表；公开与后台 API 已覆盖 |
...
- 后端 `mvn clean test` 完整验证通过：695 项测试，0 failures / 0 errors / 8 skipped；本机没有 Docker，真实 MySQL 条件测试在本地跳过，真实方言结果见下方 CI 记录。
...
- 本分支 PR #46 的 GitHub Actions [`29494102031`](https://github.com/tongchang01/My-Blog/actions/runs/29494102031) 五项检查全部通过；真实 MySQL 8.4 已验证 Flyway、改密、登录失败、评论审核和首页槽位并发测试。
- `main` 的最近一次成功 CI 为 [`29255126872`](https://github.com/tongchang01/My-Blog/actions/runs/29255126872)，对应提交 `b6d985454768c745b3dd8ec65dfb918a4784c800`。
