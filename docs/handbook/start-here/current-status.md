...
V2 的后端、公开博客主阅读链路和管理后台主要业务闭环已经实现。当前主线不是继续重建模块，而是补齐本地初始化的 reset 合约、确认生产拓扑并完成首次可回滚部署。
...
- 本地 MySQL 初始化脚本仅支持 PowerShell 7+；Windows 与 Ubuntu CI 合约已通过，显式 `-Reset` 尚未纳入合约覆盖。
...
- 本地 MySQL 合约脚本已在 Windows PowerShell 7 与 Ubuntu GitHub Actions `pwsh` 通过；显式 `-Reset` 场景仍登记为发布前事项。
