...
- Spotify 官方 Embed 第一版已实现，使用者已确认顶部音乐入口、官方标准版和移除自定义操作栏的布局；暂停使用 Spotify 控件，收起保留播放器。首次展开等待第三方加载，本版不包含预加载或加载提示。实现边界见[博客端](../frontend/blog/README.md)，真实设备、歌单更新与不同账户的后续验收由 ISSUE-010 跟踪。
