...
- 评论审核写路径已按讨论串根节点串行化，但尚待 CI 的真实 MySQL 并发回归确认。
...
- 后端 `mvn clean test` 完整验证通过：695 项测试，0 failures / 0 errors / 8 skipped；真实 MySQL 测试已覆盖评论竞争和首页槽位竞争，本机因没有 Docker 跳过，待 CI 验证。
