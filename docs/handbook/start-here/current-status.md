...
| V2 后台 admin | 基础闭环和多项业务页完成 | 登录、会话、文章、分类标签、评论、友链、附件、站点配置、作者资料、统计仪表盘已有实现材料，仍需继续校准和补齐体验边界 |
| 文档体系 | 整理中 | 正在从 `project-handbook/`、`superpowers/`、`frontend/apps/admin/docs/` 迁入 `handbook/` |
...
- 仪表盘：复用 `/api/admin/stats/dashboard` 展示真实统计，不使用伪造数字。
- 文章管理：列表、新建、编辑、状态、Markdown 预览、本地草稿、回收站、软删除和恢复入口。
- 分类/标签管理：列表、筛选、新增、编辑、删除，分类支持排序，标签不排序。
- 评论管理：已有设计、计划和验收材料，需在 handbook 中单独校准。
...
- 后台各业务页的最终完成度和自动化验证记录需要从 `frontend/apps/admin/docs/` 提炼到 `handbook/frontend/admin/`。
- DEMO 敏感字段裁剪需逐页确认。
- 友链头像附件选择器、评论回复工作流、统计图表化等体验增强仍在后续边界。

未完成项以 `open-issues.md` 的 O-002、O-005、O-006 为准。
...
1. 继续整理文档：把 `project-handbook/` 中当前有效内容迁入 `handbook/`，并归档旧计划。
2. 校准后台完成度：把 `frontend/apps/admin/docs/` 提炼为 `handbook/frontend/admin/`。
3. 补齐前台读者主链路：分类、标签、归档、友链、关于、搜索。
4. 推进后台内容生产和管理体验收口。
5. 上线准备：SEO、RSS、Sitemap、部署、备份和 CI/CD。
