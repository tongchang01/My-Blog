...
V2 的后端、公开博客主阅读链路和管理后台主要业务闭环已经实现。当前主线不是继续重建模块，而是完成本地初始化的 Linux 验证、确认生产拓扑并完成首次可回滚部署。
...
- 本地 MySQL 初始化脚本仅支持 PowerShell 7+；Windows 合约测试已通过，Linux 运行环境尚未完成同等验证。
...
- 本地 MySQL 合约脚本已在 Windows PowerShell 7 通过；当前主机未安装 Linux 运行环境，Linux 验证仍登记为发布前事项。
