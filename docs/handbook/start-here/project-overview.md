...
数据库为 MySQL 8，当前 Flyway V1–V6 建立 16 张表；V6 新增 PASSWORD 文章短期访问授权表。运行时统一 `Asia/Tokyo`。认证使用 Spring Security、JWT access token、数据库 refresh token、token version 与 BCrypt。
