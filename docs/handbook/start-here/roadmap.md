...
1. 第一版发布范围收口。
2. 前台残留旧数据源清理：作者卡片、移动菜单、友链页、页脚友链。
3. 上线部署硬项准备：服务器信息、S3、环境变量、反向代理、备份和冒烟。
4. 手动部署跑通后再设计 CD。
...
- [x] 访问统计前台打点和页脚展示。
- [x] 文章详情评论。
- [ ] 作者卡片和移动菜单脱离旧 `/authors/blog-author.json` / `/statistic.json`。
- [ ] 友链简版：接入 `GET /api/public/friend-links`，只展示公开友链列表卡片。
- [ ] 页脚友链脱离旧 `avatarWall` page 数据。
- [ ] 旧通用 page 评论入口裁决并删除活跃消费者。
- [ ] 与 V2 API 契约同步校准。
