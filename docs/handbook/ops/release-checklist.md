...
- [ ] 本周 MySQL 逻辑备份、SHA-256 校验文件和 30 天 S3 生命周期已核对；数据库迁移前再在临时库验证恢复。
...
- [ ] 主站、`www` 和管理端返回一年期 HSTS、`nosniff`、Referrer Policy、`X-Frame-Options: DENY` 和最小 Permissions Policy；公开 `/api` 正常返回业务响应并保留 Spring Security 的 `nosniff`，不依赖 Caddy 补齐页面策略。
...
- [ ] 需要数据恢复时先在隔离环境验证备份；应用版本回滚不能假定 schema 自动回滚，也不作为日常演练。
- [ ] 发布结果、异常、回滚点和后续处理已记录。
