...
> 最后校准：2026-08-09
> 对应代码：`.github/workflows/`、`deploy/cd/test/workflow-contract-test.sh`、`deploy/web/Caddyfile`、`deploy/web/test/security-headers-contract-test.sh`
> 权威程度：CI 说明
...
| Deployment workflow contract | Ubuntu `bash` + Caddy 2.11.4：镜像发布、OIDC、临时 SSH 放行、同 SHA 部署、公网冒烟，以及 Caddy 安全响应头契约与配置语法 |
...
| Deploy production | 仅 main、依赖 publish：临时放行当前 Runner 的 SSH /32，调用受限 deploy 用户部署同 SHA，校验三个公网 `/healthz`、三域静态响应头和一个公开 API 响应头，并始终撤销规则 |
...
Caddy 使用仅在响应头缺失时生效的默认值，为博客、`www`、管理端和代理响应补齐低风险基线；Spring Security 已提供的 `X-Frame-Options: DENY` 等更严格值保持不变。CI 使用 `deploy/web/Dockerfile` 固定的 Caddy 镜像验证语法，CD 在新镜像启动后读取公网响应头，避免把静态配置存在误判为生产已生效。
