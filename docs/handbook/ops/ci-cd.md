...
CI 在 main 的 pull request、push 和手动 workflow dispatch 时运行，仓库权限为 contents read。main 的 push 和版本 tag 还会触发 GHCR 镜像发布；当前仍没有自动部署或服务器操作。
...
镜像发布成功不等于生产上线。服务器侧仍需人工完成配置注入、备份/回滚点、Compose 预检、维护窗口、部署和产品冒烟。CD 只能在服务器拓扑、密钥注入、迁移、回滚和审批方式明确后单独设计。
