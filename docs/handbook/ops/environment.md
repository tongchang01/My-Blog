...
| `MYBLOG_STORAGE_LOCAL_PUBLIC_BASE_URL` | 可选 | 不使用 | 本地附件公开 URL；必须是不含用户名、密码的绝对 HTTP/HTTPS URL |
...
| `MYBLOG_STORAGE_S3_PUBLIC_BASE_URL` | 不使用 | S3 必填 | 公开资源前缀；必须是不含用户名、密码的绝对 HTTP/HTTPS URL |
