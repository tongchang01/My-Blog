...
| `V6__add_article_access_token.sql` | 新增 PASSWORD 文章访问令牌 hash、过期和撤销记录 |
...
| content | `t_article_access_token` | PASSWORD 文章令牌 hash、过期时间与撤销状态 |
...
- `t_article_access_token`：使用 `revoked`，不使用软删除三件套。
...
- `t_article_access_token.token_hash` 全局唯一，明文不持久化。
