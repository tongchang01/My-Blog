...
- `b44ea6a`：建立友链领域与持久化查询
- `fa0fc53`：实现公开与后台友链查询
- `b3be042`：实现友链新增与编辑
- `b51a1e8`：实现友链状态排序与软删除
- 完整 HTTP 与 OpenAPI 契约测试已通过；最终执行 `mvn clean test`：426 tests，0 failures，0 errors，4 skipped，跳过项均为 Docker 不可用时的 Testcontainers MySQL 条件测试。
