...
**Files:**
- Modify: `docs/project-handbook/rules/comment-style.md`
- Modify: `docs/project-handbook/rules/sql-placement.md`
- Modify: `docs/project-handbook/rules/testing-policy.md`
- Modify: `docs/project-handbook/rules/security-baseline.md`
- Modify: `docs/project-handbook/arch/persistence-strategy.md`
- Modify: `docs/project-handbook/arch/README.md`
- Modify: `docs/project-handbook/CLAUDE.md`

- [ ] **Step 1: 清除当前规则中的旧库兼容要求**

删除或改写仍被表述为当前要求的内容：Entity 必须说明“是否兼容旧库”、XML SQL 必须解释旧库字段、测试策略默认覆盖旧库字段转换、persistence strategy 仍称 V2 处于 JdbcTemplate 迁移期。历史原因只保留在 ADR-0013 和 migration 文档。

- [ ] **Step 2: 把授权规则从实现手段改为可验证目标**

将“写操作必须靠 `@PreAuthorize("hasRole('ADMIN')")`”改为：

```text
写操作必须由后端可执行授权策略强制拒绝 DEMO，并由集成测试覆盖。
可使用 SecurityFilterChain、方法级授权或 application 授权；若多层并存，必须保持语义一致。
```
...
`testing-policy.md` 不再把某次测试总数当成长期规范。改为记录验证命令、0 failures/0 errors 门槛和 Docker 条件测试解释，并链接最新 `status.md` 或阶段 review 获取当次数量。

- [ ] **Step 4: 收窄注释与 OpenAPI 强制范围**

规则明确：公开 HTTP operation 必须有 `@Operation`；高风险 request/response 字段（权限、状态、时间、nullable、枚举、敏感字段）必须有 `@Schema`；其他层只在业务原因、状态迁移、事务边界和非直观 SQL 等位置强制注释；禁止为满足数量而翻译代码。
...
```powershell
rg -n '旧库|JdbcTemplate|@PreAuthorize|最近全量结果|每个公开方法|DTO 字段' 'docs/project-handbook/rules' 'docs/project-handbook/arch' 'docs/project-handbook/CLAUDE.md'
git diff --check
git diff --stat
git status --short
```

Expected: 命中只剩历史说明或明确否定项，不再把旧迁移阶段和单一授权注解当作当前硬规则。

```powershell
git add -- 'docs/project-handbook'
git commit -m '清理后端文档规则漂移'
```

---

### Task 9: 建立公开接口 OpenAPI 描述门禁

**Files:**
- Create: `MyBlog-springboot-v2/src/test/java/com/tyb/myblog/v2/common/openapi/OpenApiOperationCoverageTest.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/common/security/SecurityProbeController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/identity/web/AuthController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/identity/web/CurrentUserController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/content/web/AdminArticleController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/content/web/AdminCategoryController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/content/web/AdminTagController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/content/web/PublicArticleController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/content/web/PublicCategoryController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/content/web/PublicTagController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/comment/web/AdminCommentController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/comment/web/PublicArticleCommentController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/comment/web/PublicGuestbookCommentController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/system/web/AdminAttachmentController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/system/web/AdminFriendLinkController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/system/web/AdminSiteConfigController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/system/web/PublicFriendLinkController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/system/web/PublicSiteConfigController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/stats/web/AdminStatsController.java`
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/stats/web/PublicPageViewController.java`
...
- Modify: `MyBlog-springboot-v2/src/test/java/com/tyb/myblog/v2/comment/web/CommentOpenApiTest.java`
- Modify: `MyBlog-springboot-v2/src/test/java/com/tyb/myblog/v2/system/web/AttachmentOpenApiTest.java`
- Modify: `MyBlog-springboot-v2/src/test/java/com/tyb/myblog/v2/system/web/FriendLinkOpenApiTest.java`
- Modify: `MyBlog-springboot-v2/src/test/java/com/tyb/myblog/v2/system/web/SiteConfigOpenApiTest.java`
- Modify: `MyBlog-springboot-v2/src/test/java/com/tyb/myblog/v2/stats/web/StatsOpenApiTest.java`

- [ ] **Step 1: 写失败的 operation 覆盖测试**

测试扫描 `com.tyb.myblog.v2` 下带 `@RestController` 的类，收集所有带 `@RequestMapping`、`@GetMapping`、`@PostMapping`、`@PutMapping`、`@PatchMapping`、`@DeleteMapping` 的方法，断言每个方法存在非空 `@Operation.summary()`。错误消息列出完整 `ClassName#methodName`，不能只断言数量。

- [ ] **Step 2: 验证测试先失败并记录缺口**

Run: `mvn '-Dtest=OpenApiOperationCoverageTest' test`

Expected: 测试列出当前缺少 `@Operation` 的 endpoint 方法。

- [ ] **Step 3: 按模块补齐 operation 语义**

依次处理 identity、content、comment、system、stats。summary 使用面向调用方的业务动作，例如“查询公开文章评论”，不能使用方法名直译。本任务只补 operation 级门禁；字段级 `@Schema` 继续由模块 OpenAPI 测试约束高风险字段，不机械注解全部字段。

- [ ] **Step 4: 运行 OpenAPI 局部验证**

Run: `mvn '-Dtest=OpenApiOperationCoverageTest,*OpenApiTest' test`

Expected: operation 覆盖测试和全部模块 OpenAPI 测试通过。
...
git diff --stat
git status --short
git add -- 'MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2' 'MyBlog-springboot-v2/src/test/java/com/tyb/myblog/v2'
git commit -m '补齐公开接口OpenAPI描述守护'
```
...
- 上一轮遗留的旧库规则、测试基线和授权手段绑定已经清理。
- 所有公开 endpoint method 都有非空 `@Operation.summary`，并由自动测试守护。
