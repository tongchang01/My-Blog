...
**Architecture:** 复用 `UserAccountRepository`、`UserProfileRepository` 和 `PasswordHashService`：补充“是否已有 ADMIN”和“创建 ADMIN”两个持久化能力，由一个事务化的 bootstrap 应用服务编排。`ApplicationRunner` 只在显式开关开启时运行；本地 profile 开启已知开发默认值，生产以临时环境变量和非 Web 运行模式调用。管理端复用个人资料页、现有改密 API 和 `sessionService.expire()`。
...
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/identity/domain/account/UserAccountRepository.java`
...
- Modify: `MyBlog-springboot-v2/src/main/java/com/tyb/myblog/v2/identity/infrastructure/persistence/repository/MyBatisUserAccountRepository.java`
...
- Produces: `boolean existsActiveAdmin()` 和 `UserAccount createAdmin(String username, String passwordHash)`，供 bootstrap 应用服务调用。
...
让 `UserAccountEntity` 继承 `BaseEntity`，移除其 `IdType.INPUT` 的重复主键字段，使 MyBatis-Plus 在 `mapper.insert` 时生成雪花 ID。向仓储端口、Mapper 和 XML 添加以下能力：
...
`MyBatisUserAccountRepository.createAdmin` 创建 `UserAccountEntity`，设置用户名、密码摘要、`AccountType.ADMIN.databaseValue()`、`tokenVersion = 0`、`loginFailCount = 0`，调用 `mapper.insert(entity)` 并验证影响行数为 1；以生成的 ID 返回 `UserAccount`。不得设置审计人或密码明文。
...
- Consumes: Task 1 的 `UserAccountRepository`、既有 `UserProfileRepository.insert`、`PasswordHashService` 和 `UserProfile.create`。
