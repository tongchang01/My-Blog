...
- [x] **Step 2: 如果本地服务可启动，执行 local profile 冒烟**
...
git commit -m "同步后端V2本地评论审计迁移结果"
```

## 实施记录

- 执行时间：2026-05-31 14:12 JST
- 目标库：`localhost:3306 / aurora`
- 执行前检查：工作区干净，`t_comment` 存在，评论数为 0；8 个审计字段和 2 个目标索引均不存在。
- 字段迁移：已通过 `docs/superpowers/specs/2026-05-31-backend-v2-comment-audit-schema.sql` 新增 `create_ip`、`user_agent`、`reviewed_by`、`review_time`、`deleted_by`、`delete_time`、`restored_by`、`restore_time`。
- 索引迁移：已新增 `idx_comment_review_delete_time` 和 `idx_comment_parent_delete_review`。
- 迁移后验证：8 个审计字段存在，2 个索引存在，评论数仍为 0。
- V2 测试结果：临时切换当前 shell 的 `JAVA_HOME` 到 `C:\Program Files\Java\jdk-17` 后执行 `mvn "-Dtest=CommentControllerTest,DatabaseCommentWriterTest,AdminCommentControllerTest,DatabaseAdminCommentReaderTest,DatabaseAdminCommentModeratorTest" test`，结果 `BUILD SUCCESS`，29 个测试通过。
- local profile 冒烟：使用 `mvn spring-boot:run "-Dspring-boot.run.profiles=local" "-Dspring-boot.run.arguments=--server.port=18080"` 启动成功，日志确认 Java 17、`local` profile、Tomcat 18080、Hikari 成功连接本地 MySQL；未出现 `t_comment` 审计字段缺失导致的 SQL 错误。冒烟进程已停止。
- 安全说明：数据库密码仅通过当前 PowerShell 进程环境变量传入，未写入文档、配置或 Git。
