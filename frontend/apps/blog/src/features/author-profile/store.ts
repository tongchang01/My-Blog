...
const createDefaultProfile = (): AuthorProfileViewModel => ({
  name: 'TYB',
  avatar: '',
  description: '一个写代码的人，偶尔记下一些想说的话。',
  location: '',
  socials: new Social(),
  articleCount: 0,
  categoryCount: 0,
...
export const useAuthorProfileStore = defineStore('author-profile', () => {
  const profile = ref<AuthorProfileViewModel>(createDefaultProfile())
  const status = ref<AuthorProfileStatus>('idle')
  let authorProfile: PublicAuthorProfileDto | null = null
  let authorRequest: Promise<PublicAuthorProfileDto> | null = null
  let activeRequest: {
    locale: SupportedLocale
  ...
  const loadAuthor = async (): Promise<PublicAuthorProfileDto> => {
    if (authorProfile) return authorProfile
    if (!authorRequest) {
      authorRequest = loadPublicAuthorProfile()
        .then(value => {
          authorProfile = value
          return value
        })
  ...
  const load = async (locale: SupportedLocale): Promise<void> => {
    if (activeRequest?.locale === locale) return activeRequest.promise
    if (loadedLocale === locale && status.value === 'ready') return

    const version = ++requestVersion
    const request = (async () => {
      status.value = 'loading'
      const next: AuthorProfileViewModel = {
        ...profile.value,
        articleCount: 0,
        categoryCount: 0,
        tagCount: 0
    ...
    activeRequest = { locale, promise: request }
    try {
      await request
    } finally {
