...
- 文章编辑器支持安全 Markdown 预览、本地草稿自动保存/恢复/清除，以及未保存修改的离开确认。
...
- 作者头像、站点 Logo 和 Favicon 已接入附件选择器：后台继续保存现有 URL 字符串字段，选择附件时写入 `publicUrl`。
- 当前不提供主题颜色、布局开关、导航菜单或站点内容块编排，这些等待后端表结构和后台配置模型扩展后再做。
...
- 文章编辑器已接入附件选择器，可从附件库选择或清除封面，保存时继续使用现有 `coverAttachmentId` 字段。
- 作者头像、站点 Logo 和 Favicon 也已复用附件选择器，但上传入口仍集中在 `/settings/attachments`。
- 当前不提供删除、批量操作、附件回收站或物理清理；这些作为后续集成步骤处理。

## 后续边界

下一阶段可继续完善评论回复工作流、友链头像附件选择器，或为统计仪表盘增加图表化展示。
