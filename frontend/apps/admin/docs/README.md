...
MyBlog V2 后台管理端位于 `frontend/apps/admin/`。当前已完成可运行的后台基础闭环，以及首个可对接后端的文章只读列表。
...
src/features/articles/   文章列表模型、展示转换、请求状态和只读页面
...
- ADMIN 与 DEMO 均可进入 `/articles/list`；页面不提供新建、编辑、删除或批量操作。
...
- 前端完整验证：43 tests、typecheck、生产构建通过。
...
下一阶段为文章详情/编辑与 Vditor，之后按模块引入分类/标签、评论审核、留言、友链、附件、站点配置和真实统计仪表盘。写操作二次确认、DEMO 按钮禁用和各业务错误码应随对应写页面实现，不在只读列表中预造空壳。
