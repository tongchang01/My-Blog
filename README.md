...
一个模块化单体的个人博客系统。后端使用 Spring Boot 3 + Java 17 承载业务，前端拆成博客前台与管理后台两个独立的 Vue 3 应用。V2 是当前主线，V1（Spring Boot 2 + Vue 3 前台 / Vue 2 后台）仅作为归档保留在仓库中。
...
- **可运行性优先**：无 Redis / RabbitMQ / Quartz 等外部中间件依赖，本机只需 JDK 17、Node 20+、MySQL 即可完整跑通。
...
- `frontend/apps/blog`：面向访客的博客前台。技术栈 Vue 3 + Vite + TypeScript + Pinia + Vue Router 4 + vue-i18n + markdown-it，源码按 `pages / features / api / stores / models` 分层。源自 Hexo 主题 `hexo-theme-aurora`，已剥离 Hexo 运行时（详见"起源"节）。
...
| 层      | 选型                                | 说明                                     |
| ------ | --------------------------------- | -------------------------------------- |
| 运行时    | Java 17 / Node 20+                | 由 Maven Enforcer 与 `engines` 字段约束      |
...
| 认证     | 自研 JWT（access + refresh）          | 参数集中在 `myblog.security.jwt`            |
| 限流     | Caffeine 本机缓存                     | 登录失败与页面访问统计的速率控制                       |
...
| 部署     | AWS EC2 单实例                       | 后端计划容器化，当前 JAR 直跑；前端 dist 由 nginx 静态托管 |
...
│     ├─ blog/                     # V2 博客前台
│     └─ admin/                    # V2 管理后台
├─ MyBlog-springboot/              # V1 后端（归档，仅供参考）
└─ MyBlog-vue/                     # V1 前端（归档，仅供参考）
```
...
- JDK 17
- Maven 3.9+
- Node 20.19+ 或 22.12+，通过 `corepack` 启用 pnpm 9
...
- 迁移脚本位于 `MyBlog-springboot-v2/src/main/resources/db/migration/`，命名遵循 Flyway 规范（`V<version>__<description>.sql`）。
...
| 命令                                                      | 覆盖范围                      |
| ------------------------------------------------------- | ------------------------- |
| `mvn test`                                              | 后端单元测试 + ArchUnit 架构约束    |
| `mvn verify`                                            | 完整构建 + 打包                 |
| `corepack pnpm --dir frontend/apps/blog run build`      | 博客前台产物构建                  |
| `corepack pnpm --dir frontend/apps/admin test`          | 管理后台单元测试（Vitest）          |
| `corepack pnpm --dir frontend/apps/admin run typecheck` | TypeScript + Vue TSC 类型检查 |
| `corepack pnpm --dir frontend/apps/admin run build`     | 管理后台产物构建                  |



任何触碰模块划分或跨模块依赖的改动，都应以 `mvn test` 的 ArchUnit 结果作为门禁。
...
> [!NOTE]
> V1 目录（`MyBlog-springboot` + `MyBlog-vue`）仅作对照与数据迁移参考，不再接受新特性；完整历史见 `archive/v1-master-2026-06-26` 分支。
